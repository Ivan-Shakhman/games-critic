...
class AuthorViewTests(TestCase):
    ...
    def test_author_detail_view(self):
        response = self.client.get(
            reverse("games_and_reviews:author-detail", args=[self.author.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.author.username)
    ...
    def test_author_update_view(self):
        response = self.client.get(
            reverse("games_and_reviews:author-update", args=[self.author.id])
        )
        self.assertEqual(response.status_code, 200)

    def test_author_delete_view(self):
        response = self.client.get(
            reverse("games_and_reviews:author-delete", args=[self.author.id])
        )
        self.assertEqual(response.status_code, 200)
...
class GameViewTests(TestCase):
    ...
    def test_game_detail_view(self):
        response = self.client.get(
            reverse("games_and_reviews:games-detail", args=[self.game.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.game.name)
    ...
    def test_game_delete_view(self):
        response = self.client.post(
            reverse("games_and_reviews:games-delete", args=[self.game.id])
        )
...
class ReviewViewTests(TestCase):
    ...
    def test_review_detail_view(self):
        response = self.client.get(
            reverse("games_and_reviews:review-detail", args=[self.review.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.review.title)
    ...
    def test_create_review_view(self):
        response = self.client.post(
            reverse("games_and_reviews:review-create", args=[self.game.id]),
            {
                "title": "New Review",
        ...
        self.assertRedirects(
            response, reverse("games_and_reviews:reviews-list")
        )
        self.assertTrue(Review.objects.filter(title="New Review").exists())

    def test_update_review_view(self):
        response = self.client.post(
            reverse("games_and_reviews:review-update", args=[self.review.id]),
            {
    ...
    def test_delete_review_view(self):
        response = self.client.post(
            reverse("games_and_reviews:review-delete", args=[self.review.id])
        )
        self.assertRedirects(
            response, reverse("games_and_reviews:reviews-list")
        )
